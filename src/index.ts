      ...
      if (
        ...
        if (typeof fieldComponent === 'object' && 'clientProps' in fieldComponent) {
          ...
          fieldComponent.clientProps = {
            get collections() {
              const autoGenerated = Array.from(allPrivilegesMap.values()).map(
              ...
              // Merge with custom privileges for collections
              const custom = Array.from(customPrivilegesRegistry.values())
              ...
              return [...autoGenerated, ...custom]
            },
            get globals() {
              const autoGenerated = Array.from(allGlobalPrivilegesMap.values()).map(
                (globalPrivileges) => ({
                  globalSlug: globalPrivileges.globalSlug,
                  globalLabel: globalPrivileges.globalLabel,
                  privileges: globalPrivileges.privileges,
              ...
              // Merge with custom privileges for globals
              const custom = Array.from(customPrivilegesRegistry.values())
                .filter((entry) => entry.type === 'global')
                .map((entry) => ({
                  globalSlug: entry.slug,
                  globalLabel: entry.label,
                  privileges: entry.privileges,
                }))
